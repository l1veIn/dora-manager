...
pub async fn install_node(home: &Path, id: &str) -> Result<NodeEntry> {
    ...
    let result = async {
        ...
        // Determine the executable path (relative to node directory)
        let build_type = meta.build.trim().to_lowercase();
        let executable = if build_type.starts_with("pip") || build_type.starts_with("uv") {
            // Python: console_scripts entry generated by pip install
            if cfg!(windows) {
        ...
        // Save enriched metadata from registry
        ...
        Ok(NodeEntry {
            ...
            description: meta_file.description,
            author: meta_file.author,
            category: meta_file.category,
            inputs: meta_file.inputs,
            outputs: meta_file.outputs,
            avatar: meta_file.avatar,
        })
    }
    .await;
