<script lang="ts">
    ...
    async function handleImport() {
        ...
        try {
            ...
            await post("/nodes/import", payload);
            ...
            toast.error(`Failed to import node: ${e.message}`);
        } finally {
            importing = false;
        }
    }
...
<Dialog.Root bind:open>
    <Dialog.Content class="sm:max-w-[425px]">
        <Dialog.Header>
            ...
            <Dialog.Description>
                Import an existing Dora node from a local directory path or a
                Git repository URL.
            </Dialog.Description>
        </Dialog.Header>
        ...
        <div class="grid gap-4 py-4">
            <div class="grid gap-2">
                <Label for="node-source" class="font-medium"
                ...
                <Input
                    id="node-source"
                    bind:value={source}
                    placeholder="e.g., /path/to/my-node or https://github.com/..."
                    autocomplete="off"
                />
            </div>

            <div class="grid gap-2">
                <Label for="node-id" class="font-medium"
                    >Node ID Override (Optional)</Label
                >
                <Input
                    id="node-id"
                    bind:value={id}
