Dora Manager 的 `nodes/` 目录内置了 **27 个节点**，覆盖了从信号采集、逻辑控制、AI 推理到存储持久化的完整数据流构建需求。这些节点通过统一的 `dm.json` 契约描述端口、配置和运行时元信息，可直接在 YAML 数据流中引用，无需额外安装（除非需要从 Hub 拉取预编译包）。节点发现顺序为：`~/.dm/nodes`（用户安装）优先于仓库内置 `nodes/` 目录，后者可通过 `DM_NODE_DIRS` 环境变量扩展。

Sources: [README.md](https://github.com/l1veIn/dora-manager/blob/main/nodes/README.md#L1-L12)

## 节点全景与分类

在深入各个节点之前，先建立整体认知。下图按功能域展示了全部 27 个内置节点的归属关系和典型数据流向：
...
| 媒体采集 | `dm-screen-capture` | Python | 屏幕截图采集 | `configurable` |
...
| AI 推理 | `dora-yolo` | Python | YOLOv8 目标检测 | `configurable` |
| 逻辑流控 | `dm-and` | Python | 布尔 AND 聚合 | `configurable` |
| 逻辑流控 | `dm-gate` | Python | 条件门控（启用/禁用直通） | `configurable` |
| 逻辑流控 | `dm-queue` | Rust | FIFO 缓冲与 flush 控制 | `streaming` |
| 就绪检查 | `dm-check-ffmpeg` | Python | 检查 FFmpeg 可用性 | `configurable` |
| 就绪检查 | `dm-check-media-backend` | Python | 检查媒体后端就绪 | `configurable` |
| 交互组件 | `dm-button` | Python | 按钮触发控件 | `widget_input` |
...
| 工具 | `dm-downloader` | Python | 模型权重下载与哈希校验 | `configurable` |
| 测试 | `dm-test-audio-capture` | Python | 固定时长麦克风采集 | — |
| 测试 | `dm-test-media-capture` | Python | 截图/录屏采集 | — |
| 测试 | `dm-test-observer` | Python | 多模态事件聚合观察器 | — |
...
**dm-microphone** 通过 `sounddevice` 库捕获麦克风输入，输出 Float32 PCM 音频流。它支持运行时设备切换——节点启动时在 `devices` 端口发布可用设备列表（JSON），UI 层可通过 `device_id` 输入端口动态切换设备。采集缓冲区由 `max_duration` 控制（默认 0.1 秒），采集到足够数据后一次性发送到 `audio` 端口。`sample_rate` 支持 8000/16000/24000/44100/48000 五档可选。
...
**dm-screen-capture** 通过 FFmpeg 后端捕获屏幕帧，编码为 PNG 或 JPEG 字节输出。支持三种采集模式：`once`（启动时截一帧退出）、`repeat`（按固定间隔连续截屏）、`triggered`（每次 `trigger` 端口收到事件时截一帧）。在 `triggered` 模式下可精确配合定时器或门控节点实现按需采集。
...
**opencv-video-capture** 是来自 Dora Hub 的社区节点，使用 OpenCV 的 `VideoCapture` 接口采集摄像头画面。通过 `tick` 端口触发帧采集，输出带 width/height/encoding 元数据的图像数组。该节点的 `dm.json` 中未声明 `ports`，端口信息来源于其 README 文档约定。
...
**dm-mjpeg** 是 Dora Manager 中仅有的两个 Rust 原生节点之一（另一个是 `dm-queue`）。它在本地启动一个基于 `axum` 的 HTTP 服务器，将 Dora 数据流中的帧以 MJPEG 格式暴露给浏览器。支持三个 HTTP 端点：`/stream`（MJPEG 持续流）、`/snapshot.jpg`（单帧快照）和 `/healthz`（健康检查）。

| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `frame` | 输入 | UInt8 | 图像字节（支持 jpeg/rgb8/rgba8/yuv420p） |

内部使用 `tokio` 异步运行时分离 Dora 事件循环和 HTTP 服务线程，通过 `watch` 通道传递帧数据。`drop_if_no_client` 配置（默认 true）在没有 HTTP 客户端连接时跳过帧处理，节省 CPU。`FrameProcessor` 通过 `max_fps` 配置实现帧率限制，避免浏览器端 CPU 过载。
...
**dm-stream-publish** 接收编码后的图像帧，通过 FFmpeg 管道将帧推送到 dm-server 媒体后端（基于 mediamtx），使 Web UI 的 `VideoPanel` 能够渲染实时视频流。它在启动时通过 HTTP API 向 dm-server 注册流，随后持续向 RTSP/RTMP 端点推送 H.264 编码数据。输出 `stream_id`（流标识符）和 `meta`（流元数据 JSON）供下游节点消费。
...
典型组合模式：`dm-screen-capture`（triggered 模式）→ `dm-stream-publish`，在 [system-test-stream.yml](https://github.com/l1veIn/dora-manager/blob/main/tests/dataflows/system-test-stream.yml#L39-L65) 中有完整示例。
...
**opencv-plot** 在基础图像上绘制边界框（bbox）、文本和标注，典型场景是目标检测结果的可视化。输入包括 `image`（基础图像数组）、`bbox`（检测框/置信度/标签）、`text`（标注文本），输出为标注后的图像。该节点沿用了 Dora Hub 社区节点的接口约定。
...
```mermaid
flowchart LR
    MIC[麦克风音频] --> VAD[dora-vad<br/>语音活动检测]
    VAD -- "截断后的语音片段" --> WHISPER[dora-distil-whisper<br/>语音转文字]
    WHISPER -- "转写文本" --> QWEN[dora-qwen<br/>LLM 文本生成]
    QWEN -- "回复文本" --> TTS[dora-kokoro-tts<br/>文字转语音]
```

### dora-vad：语音活动检测

**dora-vad** 基于 Silero VAD 模型，在连续音频流中检测语音的起止点，仅输出包含有效语音的音频片段。它持续累积音频缓冲区，当检测到语音结束（静音持续超过 `MIN_SILENCE_DURATION_MS`，默认 200ms）时才触发输出。`MAX_AUDIO_DURATION_S`（默认 75s）限制了单次输出音频的最大长度，避免内存溢出。
...
关键环境变量：`THRESHOLD`（检测阈值，默认 0.4）、`MIN_SILENCE_DURATION_MS`（静音判定时长）、`MIN_SPEECH_DURATION_MS`（最短语音时长）。
...
**dora-distil-whisper** 封装了 OpenAI Whisper 系列模型（默认 `whisper-large-v3-turbo`），将音频转写为文本。在 macOS 上自动切换为 MLX Whisper 加速推理，Linux 上使用 HuggingFace Transformers CUDA 推理。节点内置了去噪逻辑（`remove_text_noise`）和重复文本检测（`cut_repetition`），能自动过滤模型幻觉产生的常见噪声文本。
...
关键环境变量：`TARGET_LANGUAGE`（默认 english）、`MODEL_NAME_OR_PATH`、`TRANSLATE`（是否翻译模式）。节点还接受 `text_noise` 输入端口，用于从 LLM 回复中去除重复噪声。
...
**dora-qwen** 封装了 Qwen2.5 系列语言模型，支持多轮对话。它在 macOS 上使用 `llama-cpp-python`（GGUF 格式），在 Linux 上使用 HuggingFace Transformers + CUDA。模型默认为 `Qwen/Qwen2.5-0.5B-Instruct-GGUF`，可通过 `MODEL_NAME_OR_PATH` 环境变量替换为更大的模型。
...
**dora-kokoro-tts** 基于 Kokoro-82M 模型将文本转为语音。它支持中英文自动检测——当检测到中文字符时自动切换 `lang_code="z"` 的管道。输出 24kHz Float32 音频流，按句子分段发送。文本预处理会移除 `<think...</think` 标签内容（来自 LLM 推理链），确保只合成有效文本。
...
**dora-yolo** 基于 Ultralytics YOLOv8 实现实时目标检测。接受图像帧输入，输出边界框（bbox，包含坐标、置信度和类别标签）和标注后的图像。支持 `xyxy`/`xywh` 两种边界框格式。独特的 `confidence` 输入端口允许运行时动态调整检测阈值——典型的使用方式是通过 `dm-slider` 控件实时调参。

| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `image` | 输入 | UInt8 | 图像帧（bgr8/rgb8/jpeg/png） |
| `confidence` | 输入 | Float64 | 运行时置信度阈值覆盖（0.0-1.0） |
| `bbox` | 输出 | Struct Array | 边界框+置信度+类别标签 |
| `annotated_image` | 输出 | UInt8 JPEG | 标注后的图像 |

关键配置：`model`（默认 yolov8n.pt）、`format`（xyxy/xywh）、`confidence`（默认 0.25）。
...
**dm-and** 对多个布尔输入执行逻辑与（AND）运算，通常用于聚合多个就绪检查信号。它预定义了 4 个布尔输入端口（a/b/c/d），通过 `expected_inputs` 配置指定参与 AND 的输入子集。当 `require_all_seen=true`（默认）时，所有期望输入必须至少收到一次值，结果才为 true。输出包含 `ok`（布尔结果）和 `details`（JSON 格式的详细状态）。
...
**dm-gate** 实现了一个简单的"开关门"——只有当 `enabled` 输入为 `true` 时，`value` 输入的事件才会被转发到输出。当门关闭时，事件被静默丢弃。`emit_on_enable` 配置（默认 false）控制门开启时是否立即转发缓存的上一个值。对于无法直接序列化的 timer 脉冲载荷，节点会降级发送 `pa.nulls(1)` 空脉冲，确保触发式下游节点仍能工作。
...
纯输出节点。用户在 UI 中点击按钮时，`click` 端口发出一个 UTF-8 事件。典型场景是"触发下载"、"启动录制"等一次性操作。通过 `widget_input` capability 声明控件注册和事件桥接。

配置项：`label`（按钮标签，默认 "Run"）、`poll_interval`（重连间隔，默认 1000ms）。

Sources: [dm.json](https://github.com/l1veIn/dora-manager/blob/main/nodes/dm-button/dm.json#L1-L99)

### dm-slider：数值滑块

纯输出节点。用户拖动滑块时，`value` 端口发出 Float64 数值。支持 `min_val`（默认 0）/`max_val`（默认 100）/`step`（默认 1）/`default_value`（默认 50）配置。典型用例：连接到 `dora-yolo/confidence` 端口，实时调整检测阈值。
...
纯输出节点。用户切换开关时，`value` 端口发出布尔值。配置 `default_value` 控制初始状态。典型用例：控制麦克风是否启用（`dm-microphone/enabled`）。
...
| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `data` | 输入 | Binary | 待持久化的二进制载荷 |
| `path` | 输出 | UTF-8 | 写入文件的绝对路径 |
...
| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `data` | 输入 | UTF-8 | 待序列化的文本/JSON 数据 |
| `path` | 输出 | UTF-8 | 日志文件的绝对路径 |
...
| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `data` | 输入 | Binary | 音频 PCM 数据块 |
| `path` | 输出 | UTF-8 | WAV 文件的绝对路径 |
...
**dm-downloader** 是一个带 UI 反馈的下载器节点。它支持 `sha256:hex` 格式的哈希校验、`tar.gz`/`zip` 等格式的自动解压、原子写入（下载到 `.dm-tmp` 后重命名）和失败重试。生命周期：Checking → Ready/Waiting → Downloading → Verifying → (Extracting →) Done。通过 `ui` 端口向 Web UI 推送进度状态。

| 端口 | 方向 | 类型 | 说明 |
|------|------|------|------|
| `download` | 输入 | null | 触发信号 |
| `tick` | 输入 | null | 心跳保活 |
| `path` | 输出 | UTF-8 | 下载完成后的文件/目录路径 |
| `ui` | 输出 | UTF-8 JSON | Widget 状态更新 |
...
以下展示两个实际存在的数据流，演示节点如何协同工作。

### 目标检测管线：摄像头 + YOLO + 实时调参

```yaml
# 来自 demos/robotics-object-detection.yml
nodes:
  - id: webcam
...
这个管线展示了"采集 → 推理 → 存储 → 展示"的完整模式，同时通过 `dm-slider` 实现了运行时参数动态调整。
...
这是 [qwen-dev.yml](https://github.com/l1veIn/dora-manager/blob/main/tests/dataflows/qwen-dev.yml#L1-L257) 中完整管线的核心路径：麦克风采集 → VAD 检测语音 → Whisper 转写 → Qwen 生成回复 → Kokoro TTS 合成语音 → Recorder 录制音频文件 → Display 播放。每个 AI 阶段的输出同时连接到 `dm-log` 用于调试追踪，而 `dm-input-switch` 控制麦克风的启用状态。
...
Sources: [dm.json 示例](https://github.com/l1veIn/dora-manager/blob/main/nodes/dm-and/dm.json#L1-L103), [registry.json](https://github.com/l1veIn/dora-manager/blob/main/registry.json)
