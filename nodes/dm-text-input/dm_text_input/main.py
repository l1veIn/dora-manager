...
def poll_inputs(msg: dm.Message, node: Node, yaml_id: str, poll_interval_s: float):
    ...
    while RUNNING:
        try:
            ...
            print(f"[dm-text-input] poll error: {exc}", file=sys.stderr, flush=True)
...
def main():
    ...
    label = env_str("LABEL", "Prompt")
    default_value = env_str("DEFAULT_VALUE", "")
    placeholder = env_str("PLACEHOLDER", "Type something...")
    multiline = env_bool("MULTILINE")
    ...
    print(f"[dm-text-input] starting, run_id={msg.run_id}", flush=True)
    node = Node()
