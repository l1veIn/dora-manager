...
`./dev.sh` 会在首次运行时按需安装前端依赖，从仓库根目录启动 `cargo run -p dm-server`，并同时启动带热更新（HMR）的 SvelteKit 前端开发服务器。如果 `5173` 已被占用，Vite 会切到下一个可用端口并在终端里打印真实 URL。
如果 `3210` 已经被另一个 `dm-server` 占用，脚本会复用这个后端，只启动前端开发服务器；如果 `3210` 被其他进程占用，请先停止该进程再运行 `./dev.sh`。
...
**在浏览器中访问 `./dev.sh` 打印出来的前端 URL 进入可视化管理面板。后端 API 仍然运行在 [http://127.0.0.1:3210](http://127.0.0.1:3210)；如果该端口上已经有现成的 `dm-server`，脚本会直接复用它。**
